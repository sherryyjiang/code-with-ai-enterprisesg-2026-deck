# Content audit: 31 slides to 10

## Editorial standard

The workshop is 120 minutes, but the deck is not. Slide-led teaching is capped at about 30 minutes so the live app build can occupy the remaining 90. A slide earns its place only if it calibrates the room, supplies a decision participants need during the build, or hands the room into the demo.

Enduring concepts stay: iterative work, context, a clear brief, the layers behind a real app, and evidence-based review. Tool catalogues, capability speculation, multi-agent architecture, repeated loops, and post-demo lecture slides remain cut.

## Slide-by-slide disposition

| # | Original slide | Decision | V2 treatment and rationale |
| ---: | --- | --- | --- |
| 1 | Code With AI | **Keep + update** | Retains the hero, typography, and title treatment; updates the audience and workshop promise. |
...
| 11 | Plan: Control the Path | **Merge** | Planning remains one step in the loop and in the demo runbook. |
| 12 | Build: Increase Complexity Intentionally | **Cut from V2** | The earlier pilot-scope slide was useful but too abstract for a 30-minute deck. |
...
- Removed response options from the audience questions so the opening is conversational rather than framed as a poll.
- Restored the original deck’s working-loop, context-engineering, and app-anatomy concepts because each changes how participants interpret the demo.
